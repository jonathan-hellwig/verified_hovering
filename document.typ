...
= RELATED WORK
In recent years there has been a lot of work on the risks of UAVs #cite("izadi2021quantitative", "namian2021revealing", "hartmann2013vulnerability", "weibel2004safety", "altawy2016security"). While some works focus on security and privacy of UAVs @altawy2016security, other works focus on safe operation of UAVs #cite("izadi2021quantitative", "namian2021revealing", "hartmann2013vulnerability", "weibel2004safety"). Depending on the field of application different levels of safety assurance are required. For example, there have been works that prose to use drones for package delivery, traffic monitoring, emergency response @mohta2016quadcloud and precision agriculture @merkert2020managing. While some works introduce methods that are encouraged to satisfy safety requirements, this is not acceptable when operating close to humans. In this work we focus on formal analysis methods that come with provable safety guarantees. In the case of cyber-physical systems, formal verification techniques differ on the level of abstraction: verification at the level of implementation @schumann2006autonomy and verification at the level of an abstract system model. On the front of abstract model verification two main analysis methods have emerged: reachability analysis #cite("frehse2022symbolic", "althoff2021set") and theorem proving @KeYmaera.

Reachability analysis explicitly computes approximations of the set of reachable states given an initial set. These approximation of reachable sets are compared to the safety specifications. One of the main advantages of reachability analysis is that once the model is fully specified no further intervention of the user is required. However, reachability analysis is limited to finite-time horizon piecewise affine-linear dynamics and is in general undecidable #cite("althoff2021set"). Reachability techniques are usually applied during the runtime of a system because safety cannot be guaranteed ahead of time. Recent work have focused on speeding up the computation of reachable sets by using different set-representations and by using heuristics to reduce the number of steps. Tools like SpaceEx @frehse2011spaceex, Flow\* @chen2013flow, DryVR @fan2017dryvr and PHAVer @frehse2005phaver are all based on the idea of computing reachable sets. They all differ in the specification language of the model and the most recent tool DryVR does not require an explicit model of the system. DryVR makes use of recent advances in learning methods and can be used to verify black box systems by using traces of a simulated system.

Theorem proving on the other hand is a offline technique that is based on finding invariants of a system to prove that a system satisfies a safety specification. The advantage of theorem proving is that safety of a system can be proven for an infinite time horizon @KeYmaera. However, theorem proving requires a high level of expertise and is limited to small systems. Platzer et al. have developed a tool called KeYmera X @KeYmaera that makes use of differential dynamic logic to specify hybrid systems and prove safety properties. The tool makes use of Mathematica @Mathematica reasoning engine to automate parts of the safety proofs. One issue with offline theorem proving is that the real system might not exhibit the same behavior as the model used in the proof. In a follow up work, Mitsch and Platzer developed a tool called ModelPlex @mitsch2016modelplex which generates a runtime monitor of the system to detect compliance with the model.

While there are works that focus on the use of either reachability analysis or theorem proving, there are no works directly comparing the two methods in terms of feasibility on a real world system. In this work, we use a benchmark problem of a 2D quadrotor UAV following a trajectory inside a square from Safe-control-gym @yuan2021safe. We implement a model predictive controller for the quadrotor and use both KeYmera X and SpaceEx to verify the safety of the system. We compare the results of the two methods and discuss the advantages and disadvantages of each method.
